...
export class Register implements View{
    ...
    constructor(onBack:BasicCallback){
        const prompt = new Label(         {x:0, y: Sizes.text.title}, {x:640, y: 150}, "Register");
        ...
        const passwordRepeat = new TextBox(     {x:400, y:40},   GetPositionFromCenter({x:640, y:320}, {x:400, y:40}));
        const backButton = new Button(   {x:150, y:60},  GetPositionFromCenter({x:540, y:400}, {x:150, y:60}), "Back", () => onBack());
        const registerButton = new Button(   {x:150, y:60},  GetPositionFromCenter({x:740, y:400}, {x:150, y:60}), "Register", () => this.OnRegister());
